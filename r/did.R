...
model_loan_spread <- plm(update.formula(rhs_formula, log(loan_spread) ~ .), data = panel, index = c("Ticker", "loan_year"), model = "within", effect = "twoway")
model_total_covenants <- plm(update.formula(rhs_formula, total_covenants ~ .), data = panel, index = c("Ticker", "loan_year"), model = "within", effect = "twoway")
model_secured <- plm(update.formula(rhs_formula, secured ~ .), data = panel, index = c("Ticker", "loan_year"), model = "within", effect = "twoway")
